  ...
  Future<RegisterModel> register({
    ...
    required String nickname})async {
     ...
     await user.createUserWithEmailAndPassword(email: email, password: password).then((value) {
      ...
      registerModel=RegisterModel(
      name:name,email: email,uId:uid,image: image);
    ...
    await FirebaseFirestore.instance.collection(AppConstant.kUsers).doc(uId).set(registerModel.toMap());
    await FirebaseFirestore.instance.collection(AppConstant.kUsers).doc(uId).update({'uid':uId});
  }
  
  @override
  Future<RegisterModel> getUserData({required uid}) async{
     RegisterModel? userData;
     await FirebaseFirestore.instance.collection(AppConstant.kUsers).doc(uid).get().then((value) {
